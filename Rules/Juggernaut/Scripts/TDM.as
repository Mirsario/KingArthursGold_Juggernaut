...
shared class TDMSpawns : RespawnSystem
{
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (force || canSpawnPlayer(p_info))
		{
			...
			if(p_info.team == 1) // red team
			{
				p_info.blob_name = "juggernaut";
			}
			else if (p_info.team == 0) // blue team
			{
				if(p_info.blob_name!="knight" && p_info.blob_name!="crossbowman")
				{
			...
			if (playerBlob !is null)
			{
				...
				string blobName = playerBlob.getName();
				
				if(blobName == "knight")
				{
					playerBlob.server_PutInInventory(server_CreateBlob("mat_bombs"));
					//playerBlob.server_PutInInventory(server_CreateBlob("mat_bombs"));
					//playerBlob.server_PutInInventory(server_CreateBlob("food"));
				}
				else if(blobName == "crossbowman")
				{
					//playerBlob.server_PutInInventory(server_CreateBlob("mat_bombarrows"));
					//playerBlob.server_PutInInventory(server_CreateBlob("food"));
				}
				else if(blobName == "juggernaut")
				{
					
				}

				playerBlob.server_PutInInventory(server_CreateBlob("lantern"));
			}
		}
	}
...
shared class TDMCore : RulesCore
{
	...
	void Update()
	{
		...
		u32 infoTimer = 300 * getTicksASecond();
		infoTimer *= 2;
		if(getGameTime() % infoTimer == 0)
		{
			getNet().server_SendMsg("Type !info for mod info.");
		}
		else if(getGameTime() % infoTimer == infoTimer / 2)
		{
			getNet().server_SendMsg("Type !info for mod info. ");
		}
				...
				if (player.team == team_num)
				{
					...
					if (e_player !is null)
					{
						...
						if (blob_alive)
						{
							...
							if (player_blob.getName() == "crossbowman")
							{
								player_char = "a";
							}
	...
	{
		TDMPlayerInfo p(player.getUsername(), player.getTeamNum(), player.isBot() ? "knight" : (XORRandom(512) >= 256 ? "knight" : "crossbowman"));
		players.push_back(p);
		...
		if (victim !is null)
		{
			if (killer !is null && killer.getTeamNum()!=victim.getTeamNum())
			{
				...
				if(killerBlob !is null){
					if(killerBlob.getConfig()=="juggernaut"){
						killerBlob.server_Heal(1.0f);
						print("healed by killing");
					}
				}*/
			}
	...
	void CheckTeamWon()
	{
		...
		if (sudden_death)
		{
			...
			{
				rules.SetTeamWon(1);   //juggernaut wins!
	...
	{
		// give crossbowman arrows

		if (blob.getName() == "crossbowman")
		{
